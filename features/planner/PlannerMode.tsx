...
export function PlannerMode() {
  const { user } = useAuth();
  const { tasks, loading, fetchTasks, addTask, updateTask, deleteTask, toggleTaskStatus } = useTaskStore();
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      if (activeFilter === "today") {
        return !taskDate || taskDate.getTime() <= now.getTime();
      }
  ...
  const handleSaveTask = async (title: string, description: string, dueDate?: Date) => {
    if (!user) return;
    if (editingTask) {
      ...
      await addTask(user.uid, title, description, dueDate);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-4 space-y-8 pb-32">
        ...
        <div className="space-y-4 flex-1">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-white/40 text-sm">Organize your study sessions effectively.</p>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs font-bold uppercase tracking-wider text-white/40">
              ...
              <span className="text-amber-500">{stats.percentage}%</span>
            </div>
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-2 no-scrollbar">
        {(["today", "upcoming", "completed"] as FilterType[]).map((filter) => (
          <button
            key={filter}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="text-white/60 font-bold">No tasks found</h3>
      ...
      <DeleteConfirmDialog
        ...
        onConfirm={() => deletingTaskId && deleteTask(deletingTaskId)}
      />
    </div>
  );
}
